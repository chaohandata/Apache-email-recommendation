/*
 * Scala-Spark code for finding nearest neighbors for similar items recommendation using Locality Sensitive Hashing.
 * It is an alternative to ALS recommender jobs in spark.
 *
*/
...
def run() = {
	...
	*/
	...
	println("Outliers, missing values treated")
	...
	*/
	...
	println("Stemming, tokenizing, and stopwods removal done")
	...
	*/
	...
	val indexedRDD = rddVector.map{ case((hash,vector), index) => (index, vector.toSparse)} //toSparse converts Vector to SparseVector
	...
	*/
	...
	println("Writing to solr....")
	outputDF.write.format("solr").options(Map("collection" -> outputCollectionName, "commit_within"->"5000")).save()

	println("Number of recommendations calculated:")
	println(tempCountDF.count)
	println(tempCountDF.select("_2").distinct.show)
	println("Job completed")
}

// run the LSH_recommender job
run()
